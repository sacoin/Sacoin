...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #SACoinTEST2\r");
            Send(hSocket, "WHO #SACoinTEST2\r");
        } else {
            // randomly join #SACoin00-#SACoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #SACoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #SACoin%02d\r", channel_number).c_str());
        }
