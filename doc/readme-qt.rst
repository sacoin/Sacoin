...
Alternatively, install Qt Creator and open the `SACoin-qt.pro` file.

An executable named `SACoin-qt` will be built.
...
A warning for people using the *static binary* version of SACoin on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of SACoin is linked against libdb5.3.
...
installed causes SACoin-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
